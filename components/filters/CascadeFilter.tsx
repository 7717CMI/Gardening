...
export function CascadeFilter({
  ...
  const getLevelOptions = (level: number): LevelOption[] => {
    ...
    if (level === 0) {
      ...
      // Get the level 0 options to filter them out from children
      // This ensures parent-level items don't appear as sub-options
      const level0Options = getLevelOptions(0)
      const level0Items = new Set(level0Options.map(opt => opt.value))

      // Deduplicate: Use Set to track unique segment names
      // We only want to show each unique segment name once per level
      // IMPORTANT: Filter out level-0 items - they should only appear at level 0
      const seen = new Set<string>()
      const uniqueOptions: LevelOption[] = []

      finalChildren.forEach((child: string) => {
        // Only add if we haven't seen this segment name before
        // AND it's not a level-0 item (root parent item)
        // AND it's not the same as the current parent (to prevent self-reference appearing as child)
        const isLevel0Item = level0Items.has(child)
        const isCurrentParent = child === parentKey

        // Filter out: items we've seen, level-0 items, and the current parent
        if (!seen.has(child) && !isLevel0Item && !isCurrentParent) {
          seen.add(child)
          uniqueOptions.push({
            value: child,
            label: child,
