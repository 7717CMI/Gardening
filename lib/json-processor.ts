    ...
    for (let geoIdx = 0; geoIdx < geographies.length; geoIdx++) {
      ...
      if (sourceData[geography]?.[segmentType]) {
        ...
        if (isRegionSegmentType && geography !== 'Global') {
          ...
          // IMPORTANT: Only process paths where the first segment matches the current geography
          // This prevents "Europe", "Asia Pacific", etc. from being added as children of "North America"
          structurePaths.forEach(({ path: pathArray }) => {
            const segmentPath = pathArray.slice(segmentTypeIndex + 1)
            // First item in segmentPath should be the region name (e.g., "North America")
            // Only process if it matches the current geography
            if (segmentPath.length > 0 && segmentPath[0] === geography) {
              // Second item is the country/sub-region (e.g., "U.S.")
              if (segmentPath.length > 1 && segmentPath[1] && segmentPath[1].trim() !== '') {
                const countryOrRegion = segmentPath[1]
...
export async function processJsonDataAsync(
  ...
  try {
    ...
    // Also extract regions from "By Region" segment type if present
    // This allows users to select regions like "North America", "Europe", etc. as geographies
    // Note: Only extracts top-level regions, not countries/sub-segments
    const additionalGeographies: string[] = []
    for (const topLevelGeo of geographies) {
      const geoData = structureData[topLevelGeo]
      if (geoData && typeof geoData === 'object') {
        // Look for "By Region", "By State", "By Country" segment types
        const regionSegmentTypes = ['By Region', 'By State', 'By Country']
        for (const segType of regionSegmentTypes) {
          if (geoData[segType] && typeof geoData[segType] === 'object') {
            // Extract region names only (first level under "By Region")
            // Do NOT extract countries/sub-segments
            const regions = Object.keys(geoData[segType]).filter(key => {
              const value = geoData[segType][key]
              return value && typeof value === 'object' && !Array.isArray(value)
            })

            for (const region of regions) {
              if (!additionalGeographies.includes(region) && !geographies.includes(region)) {
                additionalGeographies.push(region)
              }
            }
          }
        }
    ...
    // Add the extracted regions and countries to the geographies list
    if (additionalGeographies.length > 0) {
      console.log(`Found ${additionalGeographies.length} additional geographies from region segments:`, additionalGeographies)
      geographies = [...geographies, ...additionalGeographies]
    }
    ...
    const metadata: Metadata = {
      market_name: 'Smart Indoor Gardening System Market',
